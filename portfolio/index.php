...
| http://www.gnu.org/
+----------------------------------------------------------------------+
CVS: $Id: index.php,v 1.149 2006/04/15 17:23:07 haganfox Exp $
*/
$qdig_version = '1.2.9.2';
$mtime = microtime();
$mtime = explode(" ",$mtime);
...
*/
$site_lnk_title = 'nickolas nikolic photography';  // Link text
...
*/
$copyright['txt'] = 'All media Creative Commons Attribution License';
...
*/
$chroot_dir = 'portfolio/';  // Note: You may also want to add this path to the
							 // $qdig_files location (e.g. 'portfolio/qdig-files/');
...
*/
$header['title_text_1'] = 'nickolas nikolic portfolio';  // Leading text in title
...
$header['img_name']     = FALSE;   // Include image name in title
$header['nav_links']    = TRUE;   // Include nav. links in html header.
$header['icon']         = FALSE;   // Use the qdig-camera icon.
...
*/
$ctrl_bar['enable']   = false; //TRUE;   // Enable control bar.
$dir_nav['prefs_ena'] = false;   // Display a Preferences link, which toggles
                                // the Control Bar for a more compact gallery.
$ctrl_bar['vw_ctrl']  = false;   // Enable control bar Links Style chooser. @todo
...
*/
$img_links_above     = true; //false  // Locate image links (numerals, thumbnails) @todo
...
$thmb_row['maxwidth'] = 1000;  // Approx. max. width of thumbnails row
$thmb_row['softwrap'] = 99;   // A percentage, 51 to 99
...
*/
$upr_nav['enable']    = false; //change @todo
...
$img_link['wrap']    = true;  // Link back to first image from the last one.
...
$dir_nav['choose_main_txt'] = 'View our work';
$dir_nav['choose_main_title_txt'] = 'nickolas nikolic portfolio';
$dir_nav['choose_sub_txt']  = '';
$dir_nav['choose_sub_title_txt']  = 'nickolas nikolic portfolio';
...
$excl_imgs[] = 'favicon.gif';     // included here.  Add as many of
...
function getImageFilenames($path)
{
	...
	if ($convert_GD == TRUE) {
		$img_exts = '\.jpg$|\.jpeg$|\.jpe$|\.gif$';
	} else {
		$img_exts = '\.jpg$|\.jpeg$|\.jpe$|\.gif$|\.bmp$';
	}
...
function checkForImgs($path)
{
	...
	while (($file = readdir($pwd_handle)) != FALSE) {
		...
		if ($ignore_dotfiles == TRUE && $file[0] == '.') { continue; }
		if (eregi('\.jpg$|\.jpeg$|\.jpe$|\.gif$|\.gif$|\.bmp$', $file)) {
			$has_img = TRUE;
			closedir($pwd_handle);
			return $has_img;
		}
...
function gdVersion()
{
	...
	if (eregi('/phpinfo/', ini_get('disable_functions'))) { return '1'; }
	ob_start();
	phpinfo(8);
		...
		if (! is_file($cnvrtd_img)) {
				...
				|| $height > $maxheight)
			{
				...
				if ($convert_magick == TRUE) {
					...
					imageDestroy($src_img);
					if (eregi('\.jpg$|\.jpeg$/i', $img_file) == TRUE
						&& (imageTypes() & IMG_JPG) == TRUE)
					{
					...
					{
						imagePng($dst_img, $cnvrtd_img);
					} elseif (eregi('\.gif$/i', $img_file) == TRUE
						&& (imageTypes() & IMG_GIF) == TRUE)
					{
						imageGif($dst_img, $cnvrtd_img);
					}
...
function imageTextLinks($nmrl_row)
{
	...
	if (count($imgs) > 1) {
		if ($namelinks_ena == FALSE) {
			...
			$str .= ' <div style="padding-top:'.$nmrl_row['pad_top'].';">'."\n";
			// prepare wrap data
			$links_count = 0;
			$links_wrap = -1;
			$links_wide = 0;
...
function displayImage($reqd_image, $labels)
{
	...
	$str .= " <div style=\"$pad\">\n"; //align=\"center\" removed to test @todo
...
\n <!-- table footer row of image dispaly area -->
 $tag<div style="padding-top:{$footer['pad_top']};">
